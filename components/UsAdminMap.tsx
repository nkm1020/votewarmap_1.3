...
import usStatesGeoRaw from '@/public/data/us_states_geo_sample.json';
import usCountiesGeoRaw from '@/public/data/us_counties_geo_sample.json';
...
export interface UsAdminMapProps {
...
const LEVEL_OPTIONS: Array<{ value: RegionLevel; label: string; icon: typeof MapIcon }> = [
  { value: 'state', label: '주(State)', icon: MapPinnedIcon },
  { value: 'county', label: '카운티(County)', icon: Layers3Icon },
];
...
function levelForZoom(zoom: number, countyZoomThreshold: number): RegionLevel {
  return zoom >= countyZoomThreshold ? 'county' : 'state';
}

const US_STATES_GEO = normalizeFeatureCollection(usStatesGeoRaw);
const US_COUNTIES_GEO = normalizeFeatureCollection(usCountiesGeoRaw);

const US_STATE_CODES = extractCodes(usStatesGeoRaw as unknown as GeoJsonCodeNameCollection);
const US_COUNTY_CODES = extractCodes(usCountiesGeoRaw as unknown as GeoJsonCodeNameCollection);

export default function UsAdminMap({
  statsByCode = {},
  ...
  const stateCodesRef = useRef<Set<string>>(new Set(US_STATE_CODES));
  const countyCodesRef = useRef<Set<string>>(new Set(US_COUNTY_CODES));
  ...
  useEffect(() => {
    ...
    if (map.getLayer('state-fills')) {
      map.setPaintProperty(
        'state-fills',
        'fill-color',
        buildFillExpression(statsByCode, colorsRef.current, fillMode, stateCodesRef.current),
      );
    }
    ...
    if (map.getLayer('county-fills')) {
      map.setPaintProperty(
        'county-fills',
        'fill-color',
        buildFillExpression(statsByCode, colorsRef.current, fillMode, countyCodesRef.current),
      );
    }
  ...
  useEffect(() => {
    ...
    const map = new maplibregl.Map({
      ...
      style: mapStyle,
      center: initialCenter ?? [-98.5, 39.8],
      zoom: initialZoom ?? 2.95,
      minZoom: 2.2,
      maxZoom: 8.2,
    ...
    const syncLayerVisibility = (nextLevel: RegionLevel) => {
      if (nextLevel === 'county' && !countyLoadedRef.current) {
        const countySource = map.getSource('county') as GeoJSONSource | undefined;
        countySource?.setData(US_COUNTIES_GEO);
        if (map.getLayer('county-fills')) {
          map.setPaintProperty(
            'county-fills',
            'fill-color',
            buildFillExpression(statsRef.current, colorsRef.current, fillMode, countyCodesRef.current),
          );
        }
        countyLoadedRef.current = true;
      }

      setLayerVisibility('state-fills', nextLevel === 'state');
      setLayerVisibility('state-borders', nextLevel === 'state');
      setLayerVisibility('county-fills', nextLevel === 'county');
      setLayerVisibility('county-borders', nextLevel === 'county');
    ...
    map.on('load', () => {
      ...
      map.addLayer({
        ...
        source: 'state',
        paint: {
          'fill-color': buildFillExpression(statsRef.current, colorsRef.current, fillMode, stateCodesRef.current),
      ...
      registerHoverAndClick('state-fills', 'state');
      registerHoverAndClick('county-fills', 'county');
    ...
    map.on('zoomend', () => {
      const nextLevel = levelForZoom(map.getZoom(), zoomThreshold);
