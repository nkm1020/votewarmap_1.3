...
import type { MapTooltipContext, RegionVoteMap, RegionVoteStat } from '@/components/JpAdminMap';
import jpCountryGeo from '@/public/data/jp_country_geo_sample.json';
import jpRegionsGeo from '@/public/data/jp_regions_geo_sample.json';
import jpPrefecturesGeo from '@/public/data/jp_prefectures_geo_sample.json';

const JpAdminMap = dynamic(() => import('@/components/JpAdminMap'), { ssr: false });

const TOPICS = [
  { id: 'transport', label: '교통 정책' },
  { id: 'tourism', label: '관광 정책' },
  { id: 'disaster', label: '방재 정책' },
] as const;
...
const TOPIC_BIAS: Record<TopicId, { a: string[]; b: string[] }> = {
  transport: {
    a: ['JP-KANTO', 'JP-KANSAI', 'JP-CHUBU'],
    b: ['JP-HOKKAIDO', 'JP-TOHOKU', 'JP-KYUSHU-OKINAWA'],
  },
  tourism: {
    a: ['JP-KYUSHU-OKINAWA', 'JP-HOKKAIDO', 'JP-SHIKOKU'],
    b: ['JP-KANTO', 'JP-CHUBU'],
  },
  disaster: {
    a: ['JP-TOHOKU', 'JP-CHUGOKU', 'JP-SHIKOKU'],
    b: ['JP-KANTO', 'JP-KANSAI'],
  },
};
...
function levelLabel(level: MapTooltipContext['level']): string {
  ...
  return '도도부현(Prefecture)';
}

export default function JpMapSection() {
  ...
  const countryCodes = useMemo(
    () =>
      ((jpCountryGeo.features ?? []) as CountryFeature[])
  ...
  const regions = useMemo(
    () =>
      ((jpRegionsGeo.features ?? []) as RegionFeature[])
        .map((feature) => ({
          code: String(feature.properties?.code ?? '').trim(),
          countryCode: String(feature.properties?.country_code ?? '').trim() || 'JPN',
        }))
  ...
  const prefectures = useMemo(
    () =>
      ((jpPrefecturesGeo.features ?? []) as PrefectureFeature[])
        .map((feature) => ({
          code: String(feature.properties?.code ?? '').trim(),
          regionCode: String(feature.properties?.region_code ?? '').trim(),
          countryCode: String(feature.properties?.country_code ?? '').trim() || 'JPN',
        }))
        .filter((item) => Boolean(item.code) && Boolean(item.regionCode) && Boolean(item.countryCode)),
    [],
  );
  ...
  const statsByCode = useMemo(() => {
    ...
    regions.forEach((region) => {
      map[region.code] = computeVoteStat(region.code, activeTopic, region.code);
    });
    ...
    return map;
  }, [activeTopic, countryCodes, regions, prefectures]);
  ...
  return (
    <section className="mx-auto flex w-full max-w-6xl flex-col gap-4 px-4 py-10 sm:px-6 lg:px-8">
      <header className="flex flex-col gap-3">
        <h1 className="text-2xl font-bold tracking-tight text-slate-900 sm:text-3xl">일본 행정구역 테스트 지도</h1>
        <p className="text-sm text-slate-600 sm:text-base">
          JP 샘플 경계 데이터로 국가(일본)/지방(Region)/도도부현(Prefecture) 전환과 색칠 로직을 검증합니다.
        </p>
      </header>
      ...
      <JpAdminMap
        ...
        defaultRegionLevel="region"
        initialCenter={[138.25, 37.55]}
        initialZoom={3.9}
