...
import jpCountryGeoRaw from '@/public/data/jp_country_geo_sample.json';
import jpRegionsGeoRaw from '@/public/data/jp_regions_geo_sample.json';
import jpPrefecturesGeoRaw from '@/public/data/jp_prefectures_geo_sample.json';

export type RegionLevel = 'country' | 'region' | 'prefecture';
...
export interface JpAdminMapProps {
...
const LEVEL_OPTIONS: Array<{ value: RegionLevel; label: string; icon: typeof MapIcon }> = [
  { value: 'country', label: '국가(일본)', icon: MapPinnedIcon },
  { value: 'region', label: '지방(Region)', icon: MapIcon },
  { value: 'prefecture', label: '도도부현', icon: Layers3Icon },
];

const LEVEL_LABEL: Record<RegionLevel, string> = {
  country: '국가(일본)',
  region: '지방(Region)',
  prefecture: '도도부현(Prefecture)',
};
...
const JP_COUNTRIES_GEO = normalizeFeatureCollection(jpCountryGeoRaw);
const JP_REGIONS_GEO = normalizeFeatureCollection(jpRegionsGeoRaw);
const JP_LOCAL_AUTHORITIES_GEO = normalizeFeatureCollection(jpPrefecturesGeoRaw);

const JP_COUNTRY_CODES = extractCodes(jpCountryGeoRaw as unknown as GeoJsonCodeNameCollection);
const JP_REGION_CODES = extractCodes(jpRegionsGeoRaw as unknown as GeoJsonCodeNameCollection);
const JP_LOCAL_CODES = extractCodes(jpPrefecturesGeoRaw as unknown as GeoJsonCodeNameCollection);

export default function JpAdminMap({
  statsByCode = {},
  ...
  const countryCodesRef = useRef<Set<string>>(new Set(JP_COUNTRY_CODES));
  const regionCodesRef = useRef<Set<string>>(new Set(JP_REGION_CODES));
  const localCodesRef = useRef<Set<string>>(new Set(JP_LOCAL_CODES));
  ...
  useEffect(() => {
    ...
    const map = new maplibregl.Map({
      ...
      style: mapStyle,
      center: initialCenter ?? [138.25, 37.55],
      zoom: initialZoom ?? 3.9,
      minZoom: 3.2,
      maxZoom: 9.8,
    ...
    const syncLayerVisibility = (nextLevel: RegionLevel) => {
      if (nextLevel === 'prefecture' && !localLoadedRef.current) {
        ...
        localSource?.setData(JP_LOCAL_AUTHORITIES_GEO);
        if (map.getLayer('local-fills')) {
          map.setPaintProperty(
            'local-fills',
            'fill-color',
