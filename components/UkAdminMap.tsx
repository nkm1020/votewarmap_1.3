...
import ukCountriesGeoRaw from '@/public/data/uk_countries_geo_sample.json';
import ukRegionsGeoRaw from '@/public/data/uk_regions_geo_sample.json';
import ukLocalAuthoritiesGeoRaw from '@/public/data/uk_local_authorities_geo_sample.json';

export type RegionLevel = 'country' | 'region' | 'local_authority';
...
export interface UkAdminMapProps {
...
const LEVEL_OPTIONS: Array<{ value: RegionLevel; label: string; icon: typeof MapIcon }> = [
  { value: 'country', label: '구성국', icon: MapPinnedIcon },
  { value: 'region', label: '권역', icon: MapIcon },
  { value: 'local_authority', label: '지자체', icon: Layers3Icon },
];

const LEVEL_LABEL: Record<RegionLevel, string> = {
  country: '구성국',
  region: '권역',
  local_authority: '지자체(Local Authority)',
};
...
function levelForZoom(zoom: number, regionZoomThreshold: number, localZoomThreshold: number): RegionLevel {
...
const UK_COUNTRIES_GEO = normalizeFeatureCollection(ukCountriesGeoRaw);
const UK_REGIONS_GEO = normalizeFeatureCollection(ukRegionsGeoRaw);
const UK_LOCAL_AUTHORITIES_GEO = normalizeFeatureCollection(ukLocalAuthoritiesGeoRaw);

const UK_COUNTRY_CODES = extractCodes(ukCountriesGeoRaw as unknown as GeoJsonCodeNameCollection);
const UK_REGION_CODES = extractCodes(ukRegionsGeoRaw as unknown as GeoJsonCodeNameCollection);
const UK_LOCAL_CODES = extractCodes(ukLocalAuthoritiesGeoRaw as unknown as GeoJsonCodeNameCollection);

export default function UkAdminMap({
  statsByCode = {},
  ...
  const countryCodesRef = useRef<Set<string>>(new Set(UK_COUNTRY_CODES));
  const regionCodesRef = useRef<Set<string>>(new Set(UK_REGION_CODES));
  const localCodesRef = useRef<Set<string>>(new Set(UK_LOCAL_CODES));
  ...
  useEffect(() => {
    ...
    const map = new maplibregl.Map({
      ...
      style: mapStyle,
      center: initialCenter ?? [-2.8, 54.7],
      zoom: initialZoom ?? 4.95,
      minZoom: 4.6,
      maxZoom: 9.8,
    ...
    const syncLayerVisibility = (nextLevel: RegionLevel) => {
      if (nextLevel === 'local_authority' && !localLoadedRef.current) {
        const localSource = map.getSource('local_authority') as GeoJSONSource | undefined;
        localSource?.setData(UK_LOCAL_AUTHORITIES_GEO);
        if (map.getLayer('local-fills')) {
          map.setPaintProperty(
            'local-fills',
            'fill-color',
      ...
      setLayerVisibility('region-fills', nextLevel === 'region');
      setLayerVisibility('region-borders', nextLevel === 'region');
      setLayerVisibility('local-fills', nextLevel === 'local_authority');
      setLayerVisibility('local-borders', nextLevel === 'local_authority');
    ...
    map.on('load', () => {
      ...
      registerHoverAndClick('region-fills', 'region');
      registerHoverAndClick('local-fills', 'local_authority');
    ...
    map.on('zoomend', () => {
      const nextLevel = levelForZoom(map.getZoom(), regionZoomThreshold, zoomThreshold);
